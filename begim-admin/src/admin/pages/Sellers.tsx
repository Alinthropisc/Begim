...
import { Check, X, Star, Shield, MapPin, Phone, Award, Send } from "lucide-react";
        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-up" onClick={() => setSelected(null)}>
          <div className="relative bg-[#FBF5EC] w-full md:max-w-md max-h-[90vh] md:rounded-3xl rounded-t-3xl overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="border-t border-[#C9A961]/20 p-4 pb-safe space-y-2 bg-white/50">
              {/* Write on Telegram */}
              {selected.telegram && (
                <a href={`https://t.me/${selected.telegram}`} target="_blank" rel="noopener noreferrer"
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-2xl text-sm font-semibold text-white transition"
                  style={{ background:"linear-gradient(135deg,#229ED9 0%,#1a7fb0 100%)" }}>
                  <Send className="w-4 h-4" /> Telegram'да ёзиш (@{selected.telegram})
                </a>
              )}
