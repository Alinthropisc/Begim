...
class ReviewRepository(BaseRepository[Review]):
    ...
    async def list_for_seller(self, seller_id: int, offset: int = 0, limit: int = 20):
        ...
        items_stmt = select(Review).where(*cond).order_by(desc(Review.id)).offset(offset).limit(limit)
    ...
    async def avg_for_seller(self, seller_id: int) -> tuple[float, int]:
        stmt = select(func.avg(Review.rating), func.count(Review.id)).where(Review.seller_id == seller_id, Review.is_deleted.is_(False))
