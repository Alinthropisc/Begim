...
async def cmd_start_deeplink(message: Message, command: CommandObject) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Открыть в Begim", web_app=WebAppInfo(url=url))]])
...
async def cmd_start_plain(message: Message) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Открыть Begim", web_app=WebAppInfo(url=url))]])
